...
- **Minimalist and portable:** AWSM is just a single `.html` file that is just missing your game `.wasm` encoded as a string. So, building your game involves compiling a `.wasm` game with your language of choice, base64-string-encoding that `.wasm`, then performing a string substitution to insert your cartridge-string into the `.html` file. See [Making a game](#create-and-run-an-awsm-game). All of the accompanying documentation for AWSM is found either in this README or in `index.html` as indicated by this README.
